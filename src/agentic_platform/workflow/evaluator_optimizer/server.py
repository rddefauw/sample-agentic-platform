...
app = FastAPI(title="Evaluator Optimizer Workflow")

# Essential middleware.
configuration_server_middleware(app, path_prefix="/evaluator-optimizer")

# Essential endpoints
@app.post("/search", response_model=AgenticResponse)
@handle_exceptions(status_code=500, error_prefix="Evaluator Optimizer Workflow API Error")
async def search(request: AgenticRequest) -> AgenticResponse:
    """
    Search with a langgraph evaluator optimizer workflow.
    """
    return EvaluatorOptimizerWorkflowController.search(request)
